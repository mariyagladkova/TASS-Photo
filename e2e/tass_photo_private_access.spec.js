...
test('downloading of private access images', async ({ page, context }) => {
  ...
  // Выбор Закрытого доступа
  await page.locator('form:nth-child(1) div.margin-top-15 label.radio:nth-child(3) div.radio__view').click();
  ...
  // Попытка скачать со страницы альбома (пятый по счёту)
  await newPage.locator('div.row div:nth-child(5) div.events__event').click();
  ...
  // Попытка скачать альбом из поиска
  ...
  // Попытка скачать медиафайл из фотопотока
  ...
  // Попытка скачать медиафайл со страницы просмотра альбома
  ...
  // Попытка скачать медиафайл со страницы его просмотра
  ...
  // Поле "Пароль" - ввод верного пароля
  ...
  // Проверка наличия кнопки локализации
  const languageElement = newPage.locator('a.burger__language-link');
  await expect(languageElement).toBeVisible();

  // Проверка наличия информации о пользователе
  const profileElement = newPage.locator('div.burger__header-wrapper div.burger__account-email');
  ...
  // Переход на главную
  await newPage.locator('div.page a.header__logotype').click();

  // Скачивание альбома с главной в альбомном виде
  await newPage.locator('div.row div:nth-child(1) div.events__event').hover();
  await newPage.locator('div.row div:nth-child(1) div.events__event use').click();
  //await newPage.locator('div.row div:nth-child(1) div.events__event ul li:nth-child(5) a').click();
  await newPage.getByRole('listitem').filter({ hasText: 'Оригинал' }).click();

  // Скачивание со страницы альбома (пятый по счёту)
  await newPage.locator('div.row div:nth-child(5) div.events__event').click();
  await newPage.locator('div#js-control-albums a.control__download').click();
  await newPage.getByRole('listitem').filter({ hasText: 'Оригинал' }).click();
