...
test('downloading of partially disabled authorization access images', async ({ page, context }) => {
  ...
  // Подтверждение, что чекбокс "Использовать авторизацию на проекте" не активен
  ...
  // Скачивание со страницы альбома (третий по счёту)
  await newPage.locator('div.row div:nth-child(3) div.events__event').click();
  ...
  await newPage.locator('div.events__stream-event:nth-child(5) div.events__stream-event-wrapper').hover();
  await newPage.locator('div.events__stream-event:nth-child(5) div.events__stream-event-wrapper use').click();
  await newPage.getByRole('listitem').filter({ hasText: 'Оптимальное качество' }).click();
